...
public class Assign23_03040405_116 {
	...
	public static void main(String args[]) throws IOException, FileNotFoundException {
		...
		String filename = "C:\\Siddhi\\Maths_Excelsheet\\VLab_03040405_116_4_Assign23_Siddhi.xlsx";
						...
						if (ex1 != ex3 && ex2 != ex3) {
							...
							if (ex1 != ex4 && ex2 != ex4 && ex3 != ex4) {
								...
								if (ab != 0 && ac != 0) {
									...
									int p2p3 = p2 + p3;
									int p2p4 = p2 + p4;
									if (p1p1 <= 5 && p2p3 <= 5 && p2p4 <= 5) {
										String term1 = ab + power(ch1, p1p1) + power(ch2, p2p3) + power(ch3, p2)
										...
										String Question = type + ") One of the factors of $(" + term1 + ")$ is $("
												+ term3 + ifn(c) + term4 + ")$. What is the other factor?<br>#$("
												+ term1 + ")$ या राशीचा $(" + term3 + ifn(c) + term4
												+ ")$ हा एक अवयव आहे. तर दिलेल्या पर्यायां पैकी दुसरा अवयव कोणता असेल?<br>";
										// Generate Correct answer
										String Correct_ans = "$" + term2 + "$";
										// Generate wrong options
										String wrong_ans1 = "$" + if1(a) + power(ch1, p1) + power(ch2, (p2 + 1)) + "$";
										String wrong_ans2 = "$" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, (p2 + 1))
												+ "$";
										String wrong_ans3 = "$" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, p2) + "$";
										String term5 = ab + power(ch1, p1p1) + power(ch2, (p3 + p2 + 1))
												+ power(ch3, p2) + ifn(ac) + ac + power(ch1, p1)
												+ power(ch2, (p2 + 1 + p4)) + power(ch3, p2);

										String term6 = ab + power(ch1, (p1 + p1 + 1)) + power(ch2, (p2 + 1 + p3))
												+ power(ch3, p2) + ifn(ac) + ac + power(ch1, (p1 + 1))
												+ power(ch2, (p2 + 1 + p4)) + power(ch3, p2);

										String term7 = ab + power(ch1, (p1 + p1 + 1)) + power(ch2, p2p3)
												+ power(ch3, p2) + ifn(ac) + ac + power(ch1, (p1 + 1))
												+ power(ch2, p2p4) + power(ch3, p2);
										String Solution = "​Ans :  " + Correct_ans
												+ "  <br>To find out other factor of $(" + term1
												+ ")$,<br> Let us take the multiplication of given factor $(" + term3
												+ ifn(c) + term4
												+ ")$ with all options, one by one.<br>By taking these multiplications<br>For option "
												+ wrong_ans1 + " we get<br>$\\therefore " + (term3 + ifn(c) + term4)
												+ "  \\times (" + if1(a) + power(ch1, p1) + power(ch2, (p2 + 1))
												+ ") = " + term5 + " \\ne$  the given expression.<br>For option "
												+ wrong_ans2 + " we get,<br>$\\therefore " + (term3 + ifn(c) + term4)
												+ "  \\times (" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, (p2 + 1))
												+ ") = " + term6 + " \\ne$  the given expression.<br>For option"
												+ wrong_ans3 + " we get,<br>$\\therefore " + (term3 + ifn(c) + term4)
												+ "  \\times (" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, p2)
												+ ") = " + term7 + " \\ne$  the given expression.<br>For option "
												+ Correct_ans + " we get,<br>$\\therefore " + (term3 + ifn(c) + term4)
												+ "  \\times (" + term2 + ") = " + term1
												+ " =$ given expression.<br>$\\therefore$ required other  factor is "
												+ Correct_ans + " is the answer<br>#उत्तर :  " + Correct_ans
												+ "  <br>$(" + term1
												+ ")$ या दिलेल्या राशीचा दुसरा अवयव शोधण्या साठी<br> आपण दिलेल्या प्रत्येक पर्यायाचा $"
												+ term3 + ifn(c) + term4
												+ "$ या दिलेल्या अवयव बरोबर गुणाकार करू. <br>पर्याय " + wrong_ans1
												+ " साठी आपल्याला <br>$ " + term3 + ifn(c) + term4 + "  \\times ("
												+ if1(a) + power(ch1, p1) + power(ch2, (p2 + 1)) + ") = " + term5
												+ "$ असे मिळते आणि ही राशी दिलेल्या राशी पेक्षा वेगळी आहे.<br>पर्याय  "
												+ wrong_ans2 + " साठी आपल्याला <br>$ " + term3 + ifn(c) + term4
												+ "  \\times (" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, (p2 + 1))
												+ ") = " + term6
												+ " $  असे मिळते आणि ही राशी दिलेल्या राशी पेक्षा वेगळी आहे.<br>पर्याय  "
												+ wrong_ans3 + " साठी आपल्याला <br>$ " + term3 + ifn(c) + term4
												+ "  \\times (" + if1(a) + power(ch1, (p1 + 1)) + power(ch2, p2)
												+ ") = " + term7
												+ " $ असे मिळते आणि ही राशी दिलेल्या राशी पेक्षा वेगळी आहे.<br>पर्याय  "
												+ Correct_ans + " साठी आपल्याला <br>$ " + term3 + ifn(c) + term4
												+ "  \\times (" + term2 + ") = " + term1
...
//                  			row.createCell(17).setCellValue(" ");
										row.createCell(18).setCellValue(116);
										mapsize = map.size();
										map.put(Question, i);
										...
										if (Correct_ans == wrong_ans1 || Correct_ans == wrong_ans2
												|| Correct_ans == wrong_ans3 || wrong_ans1 == wrong_ans2
												|| wrong_ans1 == wrong_ans3 || wrong_ans2 == wrong_ans3) {
